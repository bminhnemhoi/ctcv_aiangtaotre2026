...
# CTCV agent service — build context = repo root (brief D14):
#   docker compose -f deploy/docker-compose.yml --profile cpu build agent
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --no-dev --no-install-workspace --package ctcv-agent
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --no-dev --package ctcv-agent \
    || uv sync --frozen --no-dev --no-editable --package ctcv-agent
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=30s --retries=3 \
    CMD ["python", "/app/healthcheck.py", "8010", "/health"]

# Spec form: `uvicorn ctcv_agent.main:app --host 0.0.0.0 --port 8010`. serve.py resolves the
...
CMD ["python", "/app/serve.py", "ctcv_agent", "8010"]
