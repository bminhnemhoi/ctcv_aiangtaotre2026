...
# CTCV api service — build context = repo root (brief D14):
#   docker compose -f deploy/docker-compose.yml --profile cpu build api
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --no-dev --no-install-workspace --package ctcv-api
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --no-dev --package ctcv-api \
    || uv sync --frozen --no-dev --no-editable --package ctcv-api
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD ["python", "/app/healthcheck.py", "8000", "/health"]

# Spec form: `uvicorn ctcv_api.main:app --host 0.0.0.0 --port 8000`. serve.py resolves the
...
CMD ["python", "/app/serve.py", "ctcv_api", "8000"]
