...
# CTCV vision service — build context = repo root (brief D14):
#   docker compose -f deploy/docker-compose.yml --profile cpu build vision
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --no-dev --no-install-workspace --package ctcv-vision
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --frozen --no-dev --package ctcv-vision \
    || uv sync --frozen --no-dev --no-editable --package ctcv-vision

# ------------------------------------------------------------------ runtime
FROM ${PYTHON_IMAGE} AS runtime

ARG APT_PACKAGES="libgomp1 libgl1 libglib2.0-0"
RUN set -eux; \
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=60s --retries=3 \
    CMD ["python", "/app/healthcheck.py", "8030", "/health"]

# Spec form: `uvicorn ctcv_vision.main:app --host 0.0.0.0 --port 8030`. serve.py resolves the
...
CMD ["python", "/app/serve.py", "ctcv_vision", "8030"]
